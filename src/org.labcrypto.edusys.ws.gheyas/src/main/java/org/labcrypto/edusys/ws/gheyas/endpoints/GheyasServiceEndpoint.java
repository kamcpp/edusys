package org.labcrypto.edusys.ws.gheyas.endpoints;
...
import org.labcrypto.edusys.ws.gheyas.schemas.GetClassesRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetClassesResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GetCommonClassesRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetCommonClassesResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GetDebtorsRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetDebtorsResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GetGroupsRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetGroupsResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GetStudentsRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetStudentsResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GetTermsRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.GetTermsResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.GheyasClassGroupType;
import org.labcrypto.edusys.ws.gheyas.schemas.GheyasClassType;
import org.labcrypto.edusys.ws.gheyas.schemas.GheyasDebtorType;
import org.labcrypto.edusys.ws.gheyas.schemas.GheyasStudentType;
import org.labcrypto.edusys.ws.gheyas.schemas.GheyasTermType;
import org.labcrypto.edusys.ws.gheyas.schemas.SearchStudentsRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.SearchStudentsResponse;
import org.labcrypto.edusys.ws.gheyas.schemas.UpdateStudentDaneshgahFieldRequest;
import org.labcrypto.edusys.ws.gheyas.schemas.UpdateStudentDaneshgahFieldResponse;
...
public class GheyasServiceEndpoint {
	...
	public @ResponsePayload GetTermsResponse handleGetTermsRequest(
			@RequestPayload GetTermsRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
		...
		if (request.getClientDateTimeYear() >= 0
			...
			auditService.addAuditLog(token, "GetTerms", "GheyasService",
					"0.0.1", "Retrieving all terms", "",
		...
		for (GheyasTerm gheyasTerm : gheyasService.getTerms(request
				.getInstituteId())) {
	...
	public @ResponsePayload GetGroupsResponse handleGetGroupsRequest(
			@RequestPayload GetGroupsRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			DateTime clientDateTime = new DateTime(
					...
					request.getClientDateTimeSecond(),
					request.getClientDateTimeMilliSecond(), DateTimeZone.UTC);
			auditService.addAuditLog(token, "GetGroups", "GheyasService",
					"0.0.1", "Retrieving all groups", "",
					clientDateTime.getMillis(), request.getClientMacAddress(),
					request.getClientIpAddress(),
					request.getClientComputerName(),
					request.getApplicationName());
		...
		for (GheyasClassGroup gheyasClassGroup : gheyasService
				.getGroups(request.getInstituteId())) {
			...
			gheyasClassGroupType.setDescription(gheyasClassGroup
					.getDescription());
	...
	public @ResponsePayload GetClassesResponse handleGetClassesRequest(
			@RequestPayload GetClassesRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			DateTime clientDateTime = new DateTime(
					...
					request.getClientDateTimeSecond(),
					request.getClientDateTimeMilliSecond(), DateTimeZone.UTC);
			auditService.addAuditLog(token, "GetClasses", "GheyasService",
					"0.0.1", "Retrieving all classes with termCodes '"
							+ request.getTermCodes() + "' and groupCodes '"
							+ request.getGroupCodes() + "' and code '"
		...
		for (GheyasClass gheyasClass : gheyasService.getClasses(
				request.getInstituteId(), request.getGroupCodes(),
				request.getTermCodes(), request.getClassNamePattern(),
				request.isShowAll(), request.getClassName(),
				request.getClassCode())) {
			...
			gheyasClassType.setTeacherTarafHesabCode(gheyasClass
					.getTeacherTarafHesabCode());
			...
			gheyasClassType.setNumberOfStudents(gheyasClass
					.getNumberOfStudents());
	...
	public @ResponsePayload GetStudentsResponse handleGetStudentsRequest(
			@RequestPayload GetStudentsRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			auditService.addAuditLog(
					...
					"GheyasService",
					"0.0.1",
					"Retrieving all students for class '"
							+ request.getClassCode() + "'", "",
					clientDateTime.getMillis(), request.getClientMacAddress(),
					request.getClientIpAddress(),
					request.getClientComputerName(),
		...
		for (GheyasStudent gheyasStudent : gheyasService.getStudents(
				request.getInstituteId(), request.getClassCode())) {
			...
			gheyasStudentType.setTarafHesabCode(gheyasStudent
					.getTarafHesabCode());
			...
			gheyasStudentType.setEmergencyPhone(gheyasStudent
					.getEmergencyPhone());
	...
	public @ResponsePayload SearchStudentsResponse handleSearchStudentsRequest(
			@RequestPayload SearchStudentsRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			auditService.addAuditLog(
					...
					"GheyasService",
					"0.0.1",
					"Retrieving all students matching with name '"
							+ request.getName() + " and family '"
							+ request.getFamily() + "'", "",
					clientDateTime.getMillis(), request.getClientMacAddress(),
					request.getClientIpAddress(),
					request.getClientComputerName(),
		...
		for (GheyasStudent gheyasStudent : gheyasService.searchStudents(
				request.getInstituteId(), request.getName(),
				request.getFamily())) {
			GheyasStudentType gheyasStudentType = new GheyasStudentType();
			gheyasStudentType.setAddress(gheyasStudent.getAddress());
			gheyasStudentType.setBirthDate(gheyasStudent.getBirthDate());
	...
	public @ResponsePayload GetDebtorsResponse handleGetDebtorsRequest(
			@RequestPayload GetDebtorsRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			DateTime clientDateTime = new DateTime(
					...
					request.getClientDateTimeSecond(),
					request.getClientDateTimeMilliSecond(), DateTimeZone.UTC);
			auditService.addAuditLog(token, "GetDebtors", "GheyasService",
					"0.0.1", "Retrieving all debtors ...", "",
					clientDateTime.getMillis(), request.getClientMacAddress(),
					request.getClientIpAddress(),
					request.getClientComputerName(),
					request.getApplicationName());
		...
		for (GheyasDebtor gheyasDebtor : gheyasService.getDebtors(request
				.getInstituteId())) {
	...
	public @ResponsePayload UpdateStudentDaneshgahFieldResponse handleUpdateStudentDaneshgahFieldRequest(
			@RequestPayload UpdateStudentDaneshgahFieldRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			auditService.addAuditLog(
					token,
					"UpdateStudentDaneshgahField",
					"GheyasService",
					"0.0.1",
					"Updating daneshgah field for student with id: '"
							+ request.getKaramuzCode() + "' ...", "",
					clientDateTime.getMillis(), request.getClientMacAddress(),
					request.getClientIpAddress(),
					request.getClientComputerName(),
					request.getApplicationName());
		...
		gheyasService.updateStudentDaneshgahField(request.getInstituteId(),
				request.getKaramuzCode(), request.getDaneshaghFieldNewValue());
	...
	public @ResponsePayload GetCommonClassesResponse handleGetCommonClassesRequest(
			@RequestPayload GetCommonClassesRequest request)
			throws TokenIsNotAuthenticatedException,
			AuthenticationFailedException, UserIsNotAuthorizedException,
			UserIsNotActiveException, ClientInformationNeededException {
		Token token = securityService.authenticateAndAuthorizeToken(request
		...
		if (request.getClientDateTimeYear() >= 0
			...
			auditService.addAuditLog(
					...
					"GheyasService",
					"0.0.1",
					"Retrieving all common classes with termCodes '"
							+ request.getTermCodes() + "' and groupCodes '"
		...
		for (GheyasClass gheyasClass : gheyasService.getCommonClasses(
				request.getGroupCodes(), request.getTermCodes())) {
			GheyasClassType gheyasClassType = new GheyasClassType();
			gheyasClassType.setActiveDays(gheyasClass.getActiveDays());
			gheyasClassType.setDescription(gheyasClass.getDescription());
			gheyasClassType.setCode(gheyasClass.getCode());
