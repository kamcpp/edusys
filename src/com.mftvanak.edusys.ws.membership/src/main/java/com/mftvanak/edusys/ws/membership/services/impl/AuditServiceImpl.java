...
public class AuditServiceImpl implements AuditService {
  ...
  public void addAuditLog (Token token, String operation, String service, String serviceVersion, String details, String description,
    ...
    auditFacade.addAuditLog (UUID.randomUUID ().toString (), DateTime.now (DateTimeZone.UTC).getMillis (), clientDateTime, operation, token.getId (), token
        .getUser ().getPersonId (), token.getUser ().getUsername (), clientMacAddress, clientIpAddress, clientComputerName, localAddress,
        remoteAddress, service, serviceVersion, application, details, description);
  }
}
