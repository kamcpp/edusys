...
public class AuditServiceImpl implements AuditService {
  ...
  public void addAuditLog (Token token, String operation, String service, String serviceVersion, String details, String description,
    ...
    auditFacade.addAuditLog (UUID.randomUUID ().toString (), DateTime.now (DateTimeZone.UTC).getMillis (), clientDateTime, operation,
        token != null ? token.getId () : -1, token != null ? token.getUser ().getPersonId () : -1, token != null ? token.getUser ().getUsername ()
            : "null", clientMacAddress, clientIpAddress, clientComputerName, localAddress, remoteAddress, service, serviceVersion, application,
        details, description);
  }
}
